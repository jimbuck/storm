...
class Storm
{
  constructor(options)
  {
    ...
    for(let prop in options.params){      
      if(options.params[prop] instanceof Array){
        options.params[prop] = new data.OneOfEach(options.params[prop]);
      }
    }
    
    this.params = new data.ParameterCollection(options.params);
  }
  ...
  static all(values){
    return new data.OneOfEach(values);
  }
  
  static numberRange(min, count, step){
    return new data.NumberRange(min, count, step || 1);
  }
  
  static randomInt(min, max, count){
    return new data.RandomNumber(min, max, count, true);
  }
  
  static randomFloat(min, max, count){
    return new data.RandomNumber(min, max, count, false);
  }
}
